...
### 세트 구성(BOM) 정의를 코드로 관리하기 (`npm run seed:bom`)

이지어드민 재고 현황 파일에는 세트 안에 어떤 색상/부품이 몇 개씩 들어가는지 정보가 없어서, 세트 구성은 `src/inventory/bomDefinitions.js`에 직접 정의해두고 `npm run seed:bom`으로 반영합니다.

- 세트(`setSku`)는 **먼저 이지어드민 재고 현황 파일을 업로드해 등록되어 있어야** 합니다(현재 이지어드민 재고 값이 기준값으로 필요하기 때문에, 코드에서 임의로 만들지 않습니다). 아직 등록 안 된 세트는 실행 시 건너뛰고 안내 메시지가 출력됩니다.
- 이지어드민에 별도 SKU가 없는 내부 전용 부자재(예: 도구세트 속 스퀴지/스크레이퍼/케이스처럼 낱개로 팔지도, 이지어드민에서 추적하지도 않는 부품)는 `INT-`로 시작하는 임의 SKU로 새로 등록되며, 초기 재고는 0으로 시작합니다. 실제 수량은 등록 후 `/inventory.html`의 "재고 조정" 패널에서 입력하세요.
- 여러 번 실행해도 안전합니다(이미 등록된 품목/구성은 건너뛰거나 수량만 갱신).
- 새로운 세트 구성을 알려주시면 `bomDefinitions.js` 배열에 항목을 추가하고 다시 `npm run seed:bom`을 실행하는 방식으로 반영합니다.
